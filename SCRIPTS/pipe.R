...
save_pdf_path <- sprintf("%s.counts.beforenorm.pdf",gsub(".rds$","",save_seurat_obj_path))
pdf(save_pdf_path)
plot1+plot2
dev.off()
...
# counts distribution after normalization
plot1 <- VlnPlot(obj, features = "nCount_SCT", pt.size = 0.1, split.by = NULL) + NoLegend()
plot2 <- SpatialFeaturePlot(obj, features = "nCount_SCT") + theme(legend.position = "right")
save_pdf_path <- sprintf("%s.counts.afternorm.pdf",gsub(".rds$","",save_seurat_obj_path))
pdf(save_pdf_path)
plot1+plot2
dev.off()
...
obj <- FindSpatiallyVariableFeatures(obj, assay = "SCT", features = VariableFeatures(obj)[1:100],selection.method = "markvariogram")

# plot spatially variable features
top.features <- head(SpatiallyVariableFeatures(obj, selection.method = "markvariogram"), 15)
save_pdf_path <- sprintf("%s.figures.pdf",gsub(".rds$","",save_seurat_obj_path))
pdf(save_pdf_path,height=15,width=20)
SpatialFeaturePlot(obj, features = top.features, ncol = 5, alpha = c(0.1, 1))
dev.off()

saveRDS(obj,save_seurat_obj_path)
...
#nDims = 30
#obj <- RunUMAP(obj, dims = 1:nDims, return.model=TRUE) # run UMAP on PCA
